...
import (
	...
	local "github.com/buildpacks/imgutil/locallayout"
...
func testImage(t *testing.T, when spec.G, it spec.S) {
		...
		when("#FromBaseImage", func() {
			when("no platform is specified", func() {
				when("base image exists", func() {
					...
					it.After(func() {
						h.DockerRmi(dockerClient, baseImageName)
					})
	...
	when("#Kind", func() {
		it("returns locallayout", func() {
			img, err := local.NewImage(newTestImageName(), dockerClient)
			h.AssertNil(t, err)
			h.AssertEq(t, img.Kind(), "locallayout")
		})
	})
